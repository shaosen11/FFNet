...
import swin_transformer_v2 as swinv2
...
def main(args):
    ...
    # 实例化模型
    model = swinv2.__dict__[arch](new_num_classes=args.num_classes).to(device)
    ...
    if args.weights != "":
        if os.path.exists(args.weights):
            ...
            msg = model.load_state_dict(weights_dict['model'], strict=False)
            print('Pretrained weights found at {} and loaded with msg: {}'.format(args.weights, msg))
        else:
    ...
    for epoch in range(args.epochs):
        # train
        mean_loss, layer_loss, layer_acc = train_one_epoch(model=model,
                                    optimizer=optimizer,
        ...
        for i in range(len(layer_loss)):
            tb_writer.add_scalar("layer_loss_" + str(i + 1), layer_loss[i], epoch)
            tb_writer.add_scalar("layer_acc_" + str(i + 1), layer_acc[i], epoch)
        ...
        # torch.save(model.state_dict(), "./weights/swinv2_base_patch4_window16_256-pre-{}.pth".format(epoch))
...
if __name__ == '__main__':
    ...
    # 数据集所在根目录
    parser.add_argument('--data-path', type=str, default="./miniimagenet/")
